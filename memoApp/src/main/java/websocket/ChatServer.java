...
// ServerEndpoint 애너테이션으로 웹 소켓 서버를 지정한다
...
public class ChatServer {
	...
	// Collections 클래스의 synchronizedSet() 매서드는 안전하게 Set컬렉션을 생성해줌
	...
	// 클라이언트로부터 메세지를 받았을 때 실행되는 매서드를 정의
	// 클라이언트가 보낸 메시지와 클라이언트의 세션이 매개변수로 넘어옴
	// 모든 클라이언트들에게 전송. 단, 메세지 작성자는 제외시킴 
	
	@OnMessage // 메시지를 받으면 실행
	public void onMessage(String message, Session session) throws IOException {
		System.out.println("메시지 전송: " + session.getId() + ":" + message);
		synchronized(clients) {
			for (Session client : clients) { 	// 모든클라이언트들에게 메세지 전달 반복
	...
	// 클라이언트가 접속을 종료했을 때 실행되는 매서드를 정의 
	// clients에서 해당 세션을 삭제 
